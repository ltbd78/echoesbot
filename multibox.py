...
class MultiBox:
    ...
    def click(self, box, coords0, t=1, padding=.5, verbose=False):
        """
    ...
    def drag(self, box, coords0, x_drag, y_drag, dur=1, t=.5, padding=.95, verbose=False):
        """
        coords0 := the coordinate of buttons mapped with (0, 0) as reference i.e. `np.array([x1, y1, x2, y2])`
        box := index of self.locs i.e. `0`
        x_drag := drag x location relative to coords0
        y_drag := drag y location relative to coords0
        t := duration of drag
        padding := inner padding of buttons to avoid mapping errors
        verbose: prints coords
        """
        ...
        pyautogui.drag(x_drag, y_drag, duration=dur)
        pause(t)

        if verbose:
            print('coords:', x, y)
    
    def in_box(self, box, coord):
    ...
    def locate(self, box, png, confidence):
        """
